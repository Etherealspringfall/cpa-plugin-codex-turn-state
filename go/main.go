...
type pluginConfig struct {
	...
	// runner.
	//
	// ProbeManagementKey is the Bearer for /v0/management/*, and it is the only
	// key left: the offline harvester authenticates to the upstream with each
	// account's own access_token, so the former probe_api_key (a bearer for CPA's
	// /v1/responses, back when a probe drove CPA instead of calling out directly)
	// no longer has a caller and is gone.
	//
...
// ticked a box.
//
// probe_management_key and probe_base_url are absent for the same reason and
// more plainly still: they say how the next probe run authenticates
