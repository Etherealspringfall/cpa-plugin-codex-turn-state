...
提供账号与模型选择、模板状态与有效期、探测启停、代理池编辑、代理连通性检查、角色切换、模拟运行及模板清理功能。
...
桶记录包含 `auth_id`、`model`、`len`、`value`、`issued_at`、`harvested_at` 和 `attribution`。模板采用临时文件写入后重命名的方式更新；目录权限为 `0700`，文件权限为 `0600`。
