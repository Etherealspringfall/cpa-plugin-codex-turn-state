...
| 约束 | 值 | 作用 |
|---|---|---|
| 三元组冷却 | 55 分钟 | 同一 `(出口, 账号, 模型)` 最多 55 分钟一次 |
| 出口间隔 | 2 秒 | 同一桶的两个出口之间隔开 |
| 同账号串行 | 1 | 一个账号同时只有一个请求在飞 |
| 429 退避 | 10 分钟 | 拿到 429/401/403 **中止走池**，整个账号退避 |
...
账号 `i` 从 `pool[i % N]` 开始走一圈。冷却键**含出口 URL**，所以新加的代理
**立刻可试**。小时上限 = `出口数 × 账号数 × 模型数`；加代理抬高上限但**不提高瞬时
速率**（那才是 429 的成因）。
...
| `models` / `probe_accounts` / `probe_proxies` | `[]` | 探测范围与出口池 |
| `probe_management_key` | `""` | **只读**用：拉账号清单 + 下载 token |
| `probe_base_url` | `127.0.0.1:8317` | CPA 管理接口地址 |
| `probe_api_key` | `""` | **已废弃**，留空 |

**本部署线上是 `inject_mode: always`**（操作员明确指示）。

`probe_accounts` / `probe_proxies` **不是白名单**：业务替换只看「桶里有没有未过期
